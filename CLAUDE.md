...
bin/keymap-export                # dump user keymap diffs to portable/scripts/presets/keyconfig/dcc.py (headless)
...
- **Keymap.** Prefs activate Blender's built-in Industry Compatible keymap. `dcc.py` holds only user overrides on top of it (currently empty) and is the text-diffable record of keymap changes; the "+" preset button in Blender's keymap prefs writes full presets to the same directory.
